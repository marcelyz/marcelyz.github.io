...
答：一共涉及四个graph，StreamGraph、JobGraph、ExecutionGraph和物理执行图；首先在client端根据用户代码生成StreamGraph，表示最初的拓扑结构；然后做一些优化，比如operator chain 等，减少网络shuffle的开销，生成JobGraph，提交给JobManager；JM根据JobGraph生成ExecutionGraph，是用来调度的执行图，可以看作是并行化版本的JobGraph，将DAG拆分到基本的调度单元。最后JobManager根据生成的ExecutionGraph对Job进行调度，在各个TM上部署Task形成一张虚拟图，即物理执行图。
...
其他：  
    ...
    * flink 1.11引入了unaligned checkpoint，原理是将没对齐的数据存储在状态里，在恢复时重放到对应的channel中，所以依然能提供exactly-once的语义。不过也会带来一些问题，state比较大，恢复时间长，生产中测试过一次，效果不是很好。
...
- 问：checekpoint失败有遇到过吗？原因是啥，怎么解决的<br/>
...
总结：两者都是基于内存计算，都是高吞吐低延迟。不过流处理flink更优，时间语义、watermark和window等都支持的都比较完善，exactly-once两阶段提交在不同的connector组件上也比较成熟，不过由于flink的tm是在集群启动就开启的进程，任务结束时并不会关闭，所以如果系统存在内存泄漏的话，可能造成集群metaspace持续增长。批处理spark更优，RDD血缘关系容错，单点故障恢复代价低，不会导致任务全局重启。spark sql优化，性能高。
...
- 问：Checkpoint和Savepoint持久化到hdfs的文件格式  
