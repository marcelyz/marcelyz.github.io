...
特性：每个窗口都包含WindowAssigner、trigger、evictor和Window Function等，Assigner负责确定待处理元素所属的Window；Trigger决定了何时触发计算。Evictor是在用来确定怎么清除数据；Window Function主要分为两种，一种是增量计算，如reduce和aggregate，一种是全量计算，如process。
