...
- 问：flink uid是啥，有什么作用  
答：flink使用uid将state映射到operator上，这对与状态恢复至关重要。默认情况下，uid是通过遍历JobGraph并hash特定operator属性来生成运算符uid。尽管对于使用者来说很方便，但是他非常脆弱，因为JobGraph的更改会生成新的uid，为了建立稳定的映射关系，建议必须为算子设置uid。
