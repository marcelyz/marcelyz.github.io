...
答：可以理解为一个水位线，它是一种衡量Event Time的机制，就是相当于给Event Time加上一个可容忍的延迟时间，然后忍这么久再触发窗口计算。这个特别的机制，就是watermark。作用是和window一起处理乱序事件。有两种产生方式，一是标点水位线（Punctuated Watermark），数据流中每一个递增的EventTime都会产生一个Watermark。 二是定期水位线（Periodic Watermark），周期性的（一定时间间隔或者达到一定的记录条数）产生一个Watermark。
