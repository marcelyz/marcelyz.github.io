...
- **共享变量和累加器**  
答：贡献变量是指多个task共享的变量，包含两种类型(累加器Accumulator和广播变量BroadCast variable)。  
...
核心是中间层的Catalyst查询编译器，经过一系列操作，如Parser、Analyzer、Optimizer、Planner等，最终将用户编写的sql语句(或DataFrame/DataSet)转化为spark系统中执行的RDD。整体Catalyst框架具有良好的可扩展性，开发者可以根据不同的需求，灵活地添加自己的语法、解析规则、优化规则和转化策略等。  
