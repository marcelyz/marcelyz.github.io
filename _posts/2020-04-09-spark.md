...
如果实在不能减少，可通过在join/group的key中增加随机key来使shuffle后的数据均匀；如对小表增加一列skew_key，使用explode(lit((0 to n).toArray))扩容n倍；对大表也增加一列skew_key，使用[monotonically_increasing_id() % n]来增加n以内的随机数，两表的n必须相等，这样才能不丢数；n越大，从概率上说倾斜的数据分配的就越均匀，不过占用的内存也越大。<br/>
