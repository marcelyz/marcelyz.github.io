...
答：master和worker都是具体的节点；driver进程可以在任意节点(client、master、worker)，负责构建sparkContext对象，向集群管理者申请资源；executor可以理解为一个进程，持有一个线程池，每个线程执行一个task；master守护进程主要负责与worker的交互。
